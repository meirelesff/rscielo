...
The journal ID can be found between `&pid=` and `&lng` (i.e., `1981-3821`). Most `rscielo`'s functions that retrive data from journals rely on this information to work. To automatically extract an ID from the URL of a journal, one may use the `get_journal_id()` function:
...
The function outputs a `tibble` in which every bibliographic item corresponds to an observation. `get_article_footnotes()` returns a similar object, but with foonotes in the rows:
...
`rscielo` in an open-source software is aimed at researchers interested in conducting transparent and reprodutible bibliometric analysis on [Scielo](http://www.scielo.br/). If you want to contribute with code that would improve towards that goal, feel free to start a issue.
