...
#' @examples
#' \dontrun{
#' df <- get_journalMetaList(id_journal = "1981-3821")
#' summary(df)
#' }

get_journalMetaList <- function(id_journal){

  if(!is.character(id_journal) | nchar(id_journal) != 9) stop("Invalid 'id_journal'.")

  journal_links <- rScielo:::get_links(id_journal) %>% unique()
  journal_data = list()

  cat(paste0("\n\n -- Scraping articles: \n"))
  for(i in  1:length(journal_links)){
    if(i%%50 == 1){
      print(paste0("\n -- ", i," of ", length(journal_links)))
    }

    journal_link      <- journal_links[i]
    journal_data[[i]] <- rScielo:::get_xml_article(journal_link)
  }

  cat("\n\nDone.\n\n")

  class(journal_data) <- c("Scielo_articleMetaList", "list")
  journal_data
}
...
get_links <- function(id_journal){
  ...
  journal <- rvest::html_nodes(page, "center .nomodel") %>%
    rvest::html_text()
  cat(sprintf("\n\nScraping article links from: \n\n\n\t%s\n\n\n...", journal))

  page %>%
    rvest::html_nodes("b a") %>%
    rvest::html_attr("href") %>%
    lapply(rScielo:::get_internal) %>%
    unlist() %>%
    substr(56, 78) %>%
